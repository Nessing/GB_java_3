...
public class ClientHandler implements Runnable {
    private Logger log = LogManager.getLogger(ClientHandler.class);
    ...
    public static Connection getConnection() throws Exception {
        ...
        return DriverManager.getConnection("jdbc:sqlite:src/main/java/databases/clientsdatabase.db");
    }
    ...
    public boolean setNickName(String nickName) {
        ...
        if (nickName.startsWith("/exit")) {
            try {
                log.info("Client: ["+  nickName + "] is exited");
                out.writeUTF("/exit");
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        if (nickName.startsWith("/enter")) {
            ...
            log.warn("Client: ["+  nickSplit[1] + "] write password [" + nickSplit[2] + "]");
            try (PreparedStatement preparedStatement = getConnection().prepareStatement(req)) {
                ...
                while (resultSet.next()) {
                    ...
                    if (resultSet.getString("Name").equals(nickSplit[1]) &&
                        ...
                        resultSet.close();
                        log.info("Client: ["+  nickName + "] is enter chat");
                        return true;
                    }
                }
                // в ином случае вход не происходит и печатается сообщение
                log.warn("Client: [" +  nickName + "] wrote wrong name or password");
        ...
        if (nickName.startsWith("/reg")) {
            String[] nickSplit = nickName.split(" ");
            log.warn("Client: ["+  nickName + "] try registered. password [" + nickSplit[2] + "]");
            try(PreparedStatement preparedStatement = getConnection().prepareStatement(req)) {
                resultSet = preparedStatement.executeQuery();
                // проход по всей БД
                while (resultSet.next()) {
                ...
                try(PreparedStatement addNewClientDB = getConnection().prepareStatement(req)) {
                    ...
                    log.warn("Client: ["+  nickName + "] success registered");
                    ...
                    log.error("Client: ["+  nickName + "] error registration");
                    return false;
                }
    ...
    public void run() {
        try {
            ...
            log.error("Handler disconnected");
            try {
                server.sendMessageToAll("=== " + this.getNickName() + " покинул чат ===");
            } catch (IOException ex) {
                log.error("client lose server");
            }
    ...
    private void setNick() throws IOException {
        ...
        log.info("client identified nick: " + getNickName());
    }
